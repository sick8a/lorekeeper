    ...
    <div class="row mb-3">
        <div class="col-md-7">
            @if (isset($foreground) && is_array($foreground) && Config::get('lorekeeper.extensions.character_foregrounds.enabled'))
                <div class="text-center" style="position: relative; display: inline-block; {{ (isset($background) && Config::get('lorekeeper.extensions.character_backgrounds.enabled')) ? implode('; ', $background) : '' }}; background-size: cover;">
                    {{-- Foreground Overlays = TRUE --}}
                   @foreach ($foreground as $data)
                        <div style="background-image: url('{{ asset('images/data/items/foregrounds/' . $data['item_id'] . '/' . $data['tag_id'] . '/' . $data['tag_id'] . '-image.png') }}'); position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-size: cover; background-repeat: no-repeat; z-index: 2;"></div>
                    @endforeach
                    <a href="{{ $character->image->canViewFull(Auth::check() ? Auth::user() : null) && file_exists(public_path($character->image->imageDirectory . '/' . $character->image->fullsizeFileName)) ? $character->image->fullsizeUrl : $character->image->imageUrl }}" data-lightbox="entry" data-title="{{ $character->fullName }}">
                        <img src="{{ $character->image->canViewFull(Auth::check() ? Auth::user() : null) && file_exists(public_path($character->image->imageDirectory . '/' . $character->image->fullsizeFileName)) ? $character->image->fullsizeUrl : $character->image->imageUrl }}" class="image" style="position: relative; z-index: 1;" alt="{{ $character->fullName }}" />
                    </a>
                </div>
            @else
                {{-- Foreground Overlays = FALSE, show only background IF ANY --}}
                ...
                @else {{-- If no background, just show character image. --}}
